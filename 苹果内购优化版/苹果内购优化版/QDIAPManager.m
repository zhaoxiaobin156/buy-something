...
@interface QDIAPManager ()<SKPaymentTransactionObserver,SKProductsRequestDelegate>
...
@implementation QDIAPManager


//单例
static QDIAPManager *storeTool;

//单例
+(QDIAPManager *)defaultTool{
    if(!storeTool){
        storeTool = [QDIAPManager new];
...
//    NSMutableArray *set = [NSMutableArray arrayWithArray:products];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    for (int i =0; i<[productArray count]-1; i++) {
        ...
        for (int j =0; j<[productArray count]-1-i; j++) {
            
            if(([[productArray[j] price] compare:[productArray[j+1] price]] ) == NSOrderedDescending){
                
                //交换
                
                [productArray exchangeObjectAtIndex:j withObjectAtIndex:j+1];
